...
function statements001() {
    ...
    console.log(qq);
    //return;
    // node scrape0a.js lat 40.652315 lon -73.920416 radius 333 address 'one rockerfella plazza'
    wtf(qq);
    return;
...
function wtf(db, qry) {
    latitude =  qry.lat;
    longitude = qry.lon;
    oldThirty = new Date(new Date().valueOf() - 1000 * 60 * 60 * 24 * 30);
    var dev_connection_string = 'mongodb://localhost:27017/platerate';
    var connection_string =  process.env.DB_PR || dev_connection_string;
    //return;
...
function doScrape(db) {
  ...
  function processVenueResp(error, response, body) {
    ...
    if (!error && response.statusCode == 200) {
      ...
      decodeVenueResponse(("log" == "don't log what are you f in crazy"), body);
      eachVenueMenu();
    } else {
  ...
  function decodeVenueResponse(bLogfiles, json) {
...
if (bLogfiles) {
    ...
    mongo.find(
        ...
        { _id: 1, name: 1}, function (err, resultCursor) {
          ...
          function processItem(err, item) {
            ...
            reqVenueMenu(("log" == "don't log what are you f in crazzy"), item, function(err) {
  ...
  function reqVenueMenu(bLogfiles, venue, callback) {
...
if (bLogfiles) { 
      ...
      onsole.log("The " + fn + " file was saved!");
};     
...
var Test = function () {};
Test.prototype.wtf = function (db, qry, cb) {
    console.log('wtf');
    wtf(db, qry);
};
